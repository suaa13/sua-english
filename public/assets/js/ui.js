...
const I = {
  ...
  /* 太阳本体：只有实心圆盘，单条闭合子路径。
     为什么光线要拆出去单画（见下面 sunRays）：morphicons 的 buildPlan() 在两个图标的
     子路径数不一致时走 bestSurjection —— 源子路径全部映射到目标中的同一条。
     太阳原本是「圆盘 + 8 条光线」共 9 条子路径，月牙只有 1 条，
     于是 8 条光线被同时映射成 8 个新月，叠在圆盘周围变成一圈齿轮。
     把光线摘走后 src/dst 都是 1 条 → 干净的 1:1 形变。
     实心靠 CSS 的 fill，见 app.css `.ts-knob morph-icon svg path`。 */
  sun: '<path d="M7 12a5 5 0 0 1 10 0a5 5 0 0 1-10 0z"/>',
  /* 8 条光线：零面积的开放子路径，只靠 stroke 显示（设 fill 也不会有副作用）。
     径向 7.2→10.2，紧贴 r=5 的圆盘，密而不挤。
     它们不参与形变，只在切换时缩回圆盘并淡出。 */
  sunRays: '<path d="M12 1.8v3M12 19.2v3M1.8 12h3M19.2 12h3M4.79 4.79l2.12 2.12M19.21 19.21l-2.12-2.12M19.21 4.79l-2.12 2.12M4.79 19.21l2.12-2.12"/>',
